...
\subsection*{Extension 1: Center-Weighted Spatial Histogram Scoring}

Beyond the equal-weight 3x3 grid used in Task 3, we implemented a second scoring
...
For landscape images like \texttt{pic.0274.jpg} where content is spread across
the entire frame, the equal-weight variant performed better. For portrait-style
or product images, the center-weighted variant may be more effective.

\subsection*{Extension 2: Python Visual Result Viewer}

We wrote \texttt{show\_matches.py}, a Python script that calls the compiled C++
\texttt{match} binary, parses the ranked filenames from stdout, and displays the
target image alongside the top $N$ matches in a single \texttt{matplotlib} figure.
This removes the need to manually look up each filename after a query.
