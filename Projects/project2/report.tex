...
\textbf{\underline{The code artifact is also submitted as a zip file. Instructions for building and running the code are provided in \texttt{README.md}.}}
...
conversion; no \texttt{cv::Sobel} or \texttt{cv::calcHist} was used. Each pixel's
gradient magnitude $M = \sqrt{G_x^2 + G_y^2}$ is binned into 16 equally spaced
bins across the range $[0, 510 \cdot \sqrt{5}]$.
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.22\textwidth}
        ...
        \caption{Match 2\\pic.0605\\score: $-0.823$}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.22\textwidth}
        \includegraphics[width=\textwidth]{report_imgs/pic.0708.jpg}
        \caption{Match 3\\pic.0708\\score: $-0.823$}
    \end{subfigure}
    \caption{Task 4: Top 3 matches for \texttt{pic.0535.jpg} using color
    + texture (Sobel gradient magnitude) histograms with equal weighting.}
\end{figure}
...
\begin{figure}[H]
    ...
    cosine distance. The results make sense because the DNN has learned to group
...
We compare ResNet18 cosine distance against 3D histogram intersection on two target
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Task 6b: \texttt{pic.0948.jpg} (DNN top hits 0930 and 0960) are
...
\subsection*{Extension 1: Center-Weighted Spatial Histogram Extension}
...
\subsection*{Extension 3: Interactive PyQt5 GUI}

We built a full interactive desktop GUI using PyQt5 (\texttt{gui.py}) that makes
the system significantly more user-friendly than a command line. The GUI has:

\begin{itemize}
    \item A file browser to select any target image from disk
    \item A directory picker for the image database (defaults to \texttt{olympus/})
    \item A dropdown for all seven methods (\texttt{baseline} through \texttt{custom})
    \item A spinbox for $N$ (1 to 20 results)
    \item A \textbf{Find Matches} button that runs the C++ binary in a background
          thread so the UI stays responsive during matching
    \item A scrollable horizontal results panel showing the target (blue border)
          and each match as a 200px thumbnail card with filename and score
\end{itemize}

All computation is still done in C++. The GUI is purely a frontend.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\textwidth]{report_imgs/gui_screenshot.png}
    \caption{Extension 3: PyQt5 GUI running the \texttt{dnn} method on
    \texttt{pic.0164.jpg} with $N=5$. The target image (blue border, left)
    is separated from the top 5 matches by a vertical line. Each card shows
    a thumbnail, filename, rank, and cosine distance score.}
\end{figure}
...
\begin{itemize}
    ...
    \item Shyam Sreenivasan implemented Tasks 1 through 3: the baseline 7x7
        ...
        scoring (Task 3). Shyam also led the overall project setup and code
        organization, designing the pipeline architecture with clean
        abstractions (\texttt{Featurizer}, \texttt{SimilarityScoring},
        \texttt{NamedImage}) and separating concerns into reusable,
