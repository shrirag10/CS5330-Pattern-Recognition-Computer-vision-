...
\textit{[Screenshot to be added by Shyam: press \texttt{f} in
\texttt{vidDisplay}, then \texttt{s} to save.]}

%------------------------------------------------------------
\subsection*{Required Images 8 and 9 -- Depth Estimation (Task 11)}
%------------------------------------------------------------

\textit{[Screenshots to be added by Shyam: press \texttt{d} for the depth
map and \texttt{c} for the depth-defocus creative filter.]}

%------------------------------------------------------------
\subsection*{Required Images 10 to 12 -- Custom Effects (Task 12)}
%------------------------------------------------------------

\textit{[Screenshots to be added by Shyam: press \texttt{n}, \texttt{e},
and \texttt{o} respectively.]}
...
\begin{itemize}
    \item Working at the pixel level with \texttt{cv::Mat} made me realize
        ...
        to those numbers.
    \item Writing two versions of the blur filter and timing them taught me
        that algorithmic decisions matter more than I expected. Switching from
    ...
    \item The Sobel filter was initially confusing because Sobel X detects
        ...
        through that helped me understand the relationship between the direction
        of a gradient and the orientation of edges.
    \item Using signed 16-bit output for the Sobel functions was something I
        had not thought about before. Without it, negative gradient values
        would be clipped silently and the results would be wrong.
    \item Integrating Depth Anything V2 through the ONNX runtime showed me
    ...
    \item Face detection with a Haar cascade was fast and surprisingly easy
        to wire in, which made me curious about how much more accurate but
        slower methods like deep learning detectors compare in practice.
    \item Overall this project changed how I look at filters in tools like
...
\begin{itemize}
    ...
    \item Shyam Srinivasan implemented Tasks 1 through 3 and Tasks 10
        ...
        custom effects).
    \item We used an AI assistant (Antigravity, powered by Claude from
        Anthropic) to help implement the filter functions in
        \texttt{filter.cpp}, set up the unified \texttt{vidDisplay.cpp}
        loop, and configure the CMake build. The algorithm details
        (kernel values, separable decomposition, Sobel signs, quantization
        formula) all came directly from the assignment specification.
\end{itemize}
