...
% ============================================================
\section*{Final Project Proposal: Gesture-Controlled Augmented Reality Interface}
% ============================================================

For the final project, I propose to design and implement a hand-gesture-controlled augmented reality (AR) interface. The system aims to replace physical checkerboard markers with natural hand gestures to manipulate virtual 3D overlays in real time.

\subsection*{Methodology}
The pipeline consists of three core components:
\begin{enumerate}
    \item \textbf{Hand Tracking and Pose Estimation}: I will use a lightweight keypoint detection network (such as MediaPipe Hands or a custom mobile-sized CNN) to extract 21 3D hand landmarks from a monocular video feed.
    \item \textbf{Gesture Recognition Classification}: The extracted landmark sequence will be fed into a temporal classifier (either a simple LSTM or a small Transformer encoder block) to classify static and dynamic hand gestures (e.g., swiping, pinching, rotating).
    \item \textbf{Interactive 3D AR Engine}: Based on the recognized gesture, the program will dynamically adjust the scale, rotation, and translation of a virtual 3D object projected onto a detected flat surface. The surface orientation and camera pose will be tracked markerlessly using ORB features and a RANSAC-based homography solver.
\end{enumerate}

\subsection*{Data Sources and Deliverables}
I will leverage publicly available hand keypoint datasets (such as the Freihand dataset) to verify keypoint extraction. I will collect a small, custom dataset of gesture video clips to train the temporal classifier. The final deliverables will include a real-time Python application executing the gesture detection and AR projection, a technical demonstration video, and a comprehensive project report.
